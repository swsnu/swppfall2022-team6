...
class PostTestCase(TestCase):
    ...
    def setUp(self):
        ...
        main_badge=badge)
        new_post = Post(user=new_user, content='content', latitude=36.0, longitude=128.0)
        new_post.save()
        new_post2 = Post(user=new_user, content='content', latitude=5.0, longitude=12.0)
        new_post2.save()
    ...
    def test_get(self):
        client = Client()
        response = client.get('/post/', {'radius': 143, 'latitude': 37.0, 'longitude': 127.0})
        
        print(str(response.content, encoding='utf8'))
        self.assertEqual(response.status_code, 200)
        self.assertJSONEqual(
            str(response.content, encoding='utf8'),
            [{'id': 1, 'user': 1, 'content': 'content', 'image': None, 'latitude': 36.0, 'longitude': 128.0, 'reply_to': None, 'hashtags': [{'id': 1, 'content': 'hashtag'}]}]
        )
